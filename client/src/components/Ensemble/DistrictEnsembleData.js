...
function districtEnsembleData({ district, districtEnsembleData }) {
  ...
  return (
    ...
    <div>
      {district !== -1 && districtEnsembleData && (
        <div className="districtDataFont">
          ...
          {parseInt(districtEnsembleData.districts[district]["district"])} <br />
          <span style={{ fontWeight: 550, fontSize: 15 }}>
            {" "}
            Projected Winner:{" "}
          </span>
    ...
    {district !== -1 && districtEnsembleData && <ReactApexChart
            options={barOptions}
            series={[{
              name: 'Percent Change',
              data: [districtEnsembleData.districts[district]["wht_diff"], districtEnsembleData.districts[district]["blk_diff"],  
              districtEnsembleData.districts[district]["hsp_diff"],  districtEnsembleData.districts[district]["asn_diff"]
              ]
            }]}
            type="bar"
            height="400"
      ...
      {district !== -1 && districtEnsembleData && <ReactApexChart
            options={pieOptions}
            series={[districtEnsembleData.districts[district]["rep_split"], districtEnsembleData.districts[district]["dem_split"]]}
            type="pie"
            height="400"
            width="400"
      />
